...
fig <- leaflet_basis(geo) %>% 
  leaflet_text("Ausschließlich geringfügig entlohnte Beschäftigte, Juni 2021", size=20, group="2021") %>%
...
fig <- fig  %>% 
  leaflet_text("Ausschließlich geringfügig entlohnte Beschäftigte, Juni 2020", size=20, group="2020") %>%
  leaflet_text(lat=55.25, 
               "Anteil an allen Beschäftigten, in Prozent", size=15) %>%
  leaflet_legend(colors = colfunc(levels(geo$gruppe)),
                 lvl = levels(geo$gruppe),
...
fig <- fig %>%
  leaflet_text("Ausschließlich geringfügig entlohnte Beschäftigte, Juni 2019", size=20, group="2019") %>%
  leaflet_polygons(
    labvar = 1, 
    colors=colfunc(levels$geo_gruppe),
    bg = colfunc(levels(geo$gruppe)),
...
fig <- fig %>%
  leaflet_text("Ausschließlich geringfügig entlohnte Beschäftigte, Juni 2018", size=20, group="2018") %>%
  leaflet_polygons(
    labvar = 1, 
    colors=colfunc(levels$geo_gruppe),
    bg = colfunc(levels(geo$gruppe)),
...
fig <- fig %>%
  leaflet_text("Ausschließlich geringfügig entlohnte Beschäftigte, Juni 2017", size=20, group="2017") %>%
  leaflet_polygons(
    labvar = 1, 
    colors=colfunc(levels$geo_gruppe),
    bg = colfunc(levels(geo$gruppe)),
