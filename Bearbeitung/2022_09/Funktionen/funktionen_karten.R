...
static_map <- function(geo = geo, titel = "", untertitel = "", caption = "", leg = "") {
  karte <-
    ...
    guides(
      fill = guide_legend(ncol = 1, title = NULL, byrow = T)
    )
...
FormatDecimal <- function(x, k) {
  temp <- NULL
  for(j in 1:length(k)) {
    temp[j] <- format(round(as.numeric(x[j]), k[j]), nsmall = k[j])
  }
  return(temp)
}
# Labels leaflet----
...
leaflet_text <- function(.data, text, lng = 3.9, lat = 55.5, size = 22, group=NULL) {
...
leaflet_polygons <-
  function(.data,
    ...
    # Labels
    if(is.null(labtext)) labtext <- headers[labvar]
    
    labs <- lapply(seq(nrow(geo)), function(i) {
      paste0( "<p style='font-weight:bold;'>", "", geo[["GEN"]][i], "", '<p></p>', 
              labtext, ": ",
              str_replace(FormatDecimal(geo[[variablen[labvar]]][i], 
                                        # falls mehrere Nachkommastellen
                                        k=k[1]), fixed("."), ","),  " ", 
              # falls mehrere Suffixe
              suffix[1],
              '</p>' ) 
    })
    ...
    for(i in 1:length(geo$GEN)) {
      ...
      popup_text[i] <- paste0(
        " <span style='vertical-align:0px; width: 200px;''>",
        headers, ":",
        "</span>",
        paste0(
