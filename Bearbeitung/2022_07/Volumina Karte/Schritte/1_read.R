...
dat <-
  ...
  mutate(AGS = if_else(nchar(AGS) == 2, paste0(AGS, "000"), AGS)) %>%
  ...
  left_join(
    read.xlsx(
      "./Input/Arbeitszeiten 2022 Helge.xlsx",
      sheet = "Tabelle1", startRow = 2, colNames = F, 
      cols=sort(c(1:6))
    ) %>%
      as_tibble() %>%
      rename(AGS = X1, Kreis = X2, 
             `Gesamt (2020)` = X3, `Verarbeitendes Gewerbe (2020)` = X4, 
             `Baugewerbe (2020)` = X5, `Öffentliche Dienstleister (2020)` = X6
      ) %>%
      filter(str_detect(AGS, "[:digit:]{5}") %in% T | AGS %in% c("02", "11")) %>%
      select(AGS, Kreis, everything()) %>%
