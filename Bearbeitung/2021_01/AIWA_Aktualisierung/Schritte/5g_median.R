...
names(dat) <- c("Kreis", "AGS", "zahl1", "zahl2", "zahl3")
...
longlatcoor$zahl1.g <- cut2(longlatcoor$zahl1, 
                            cuts=c(0,2750, 3000,3250,3500,10000))
longlatcoor$zahl1.g <- factor(longlatcoor$zahl1.g, 
                              labels=c( "bis 2750", "2751 bis 3000", "3001 bis 3250", 
                                        "3251 bis 3500", "3501 und mehr"))
...
# Farbpalette
colfunc <- colorRampPalette(farben(col_med))
...
labs <- lapply(seq(nrow(longlatcoor)), function(i) {
  paste0( "<p style='font-weight:bold;'>", "", longlatcoor[["GEN"]][i], "", '<p></p>', 
          ...
          "; Männer: ",
          str_replace(FormatDecimal(longlatcoor[["zahl2"]][i], k=0), fixed("."), ","),  " ",
          "; Frauen: ",
          str_replace(FormatDecimal(longlatcoor[["zahl3"]][i], k=0), fixed("."), ","),  " ", 
          "", '</p>' ) 
})
...
addLabelOnlyMarkers(
  ...
  addLabelOnlyMarkers(
    ...
    label =  htmltools::HTML(
      paste(
        "<font face='Arial'>",
        "Mittlere monatliche Brutto- <br>  ", 
        "",
        " arbeitsentgelte der sozialver- <br> sicherungspflichtig Vollzeitbe- <br> schäftigten",
        "",
        "",
        " der Kerngruppe, in Euro",
        "</font>", 
        sep=""
      )
    ),
...
addPolygons(
  ...
  popup=paste(
    ...
    str_replace(FormatDecimal(longlatcoor$zahl1, k=0), fixed("."), ","),  " Euro", "<br>",
    paste( 
    ...
    str_replace(FormatDecimal(longlatcoor$zahl2, k=0), fixed("."), ","),  " Euro", "<br>",
    paste( 
      longlatcoor$anmerkung, sep=""
    ), 
    "<div style=' background-color:",
    ...
    "</div>",
    "<div style=' background-color:white;color:black;padding-left:0px; padding-top: 0px; padding-right: 5px; width:195px;height:16px; '>",
    str_replace(FormatDecimal(longlatcoor$zahl3, k=0), fixed("."), ","),  " Euro", "<br>",
    paste( 
      longlatcoor$anmerkung, sep=""
    ), 
    "</div>",
