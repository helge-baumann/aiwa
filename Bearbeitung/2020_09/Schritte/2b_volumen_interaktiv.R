...
titel <- "Jahresarbeitszeit pro Arbeitnehmer, 2017"

unter <- paste0("Standardarbeitsvolumen ", "<br>", 
                "in Arbeitsstunden")
...
longlatcoor <- merge(longlatcoor, dat_vol, by="AGS")
#geo <- geo[geo$GF > 1,]
...
addPolygons(
  ...
  popup=paste(
    ...
    str_replace(FormatDecimal(longlatcoor$zahl1, k=1), fixed("."), ","),  " Stunden", "<br>",
    paste( 
