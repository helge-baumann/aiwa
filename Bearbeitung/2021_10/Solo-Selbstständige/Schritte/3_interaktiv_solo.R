...
fig <- leaflet_basis(geo) %>% 
  leaflet_text("Solo-Selbstständige in den Bundesländern, 2020", size=20, group="Gesamt") %>%
  ...
  leaflet_legend(colors = colfunc(levels(geo$gruppe)),
                 ...
                 group="Gesamt") %>%
  leaflet_logo() %>%
  leaflet_quelle(Text="Quelle: <br> Sonderauswertung des <br> Statistischen Bundesamts") %>%
...
fig <- fig %>% 
  leaflet_text("Solo-Selbstständige in den Bundesländern, 2020 (Männer)", size=20, group="Männer") %>%
  leaflet_text(lat=55.25, 
...
fig <- fig %>% 
  leaflet_text("Solo-Selbstständige in den Bundesländern, 2020 (Frauen)", size=20, group="Frauen") %>%
  leaflet_text(lat=55.25, 
