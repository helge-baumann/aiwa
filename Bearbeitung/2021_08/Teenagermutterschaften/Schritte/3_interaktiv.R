...
fig <- leaflet_basis(geo) %>% 
  leaflet_text("Teenagermutterschaften in Deutschland", size=20, group="2019") %>%
  leaflet_text(lat=55.25, 
               "Geburten unter 1.000 15- bis 19-Jährigen (2019)", size=15) %>%
  leaflet_legend(colors = colfunc(levels(geo$gruppe)),
                 lvl = levels(geo$gruppe),
                 group="2019") %>%
  leaflet_logo() %>%
  leaflet_quelle(Text="Quelle: <br> Statistische Ämter  <br> des Bundes und der Länder / <br> Statistik der Bundesagentur für Arbeit") %>%
  ...
  leaflet_polygons(
    ...
    variablen=c("Anteil (2019)", "Anzahl (2019)", "SGB-II-Quote (2019)"),
    headers=c("Teenagermutterschaften (2019)", "Anzahl Geburten (2019)", "SGB-II-Quote (2019)"),
