...
dat_ul <- dat_ul %>% rename("Anteil" = 3)
geo <- merge(geo_base, dat_ul, by=key) # dat siehe 1_read.R

# Farbenpalette: Woran geknüpft?
colfunc <- colorFactor(farben("tuerkis"), domain=geo$`Gruppe 2020`, ordered=T)
...
fig <- leaflet_basis(geo) %>% 
  leaflet_text("Unterer Entgeltbereich, 2020", size=20, group="2020") %>%
  leaflet_text(lat=55.25, 
               "<div style='padding-top: 2%; line-height:90%'>Anteil der sozialversicherungspflichtig Vollzeitbeschäftigten der Kerngruppe <br> mit einem monatlichen Bruttoarbeitsentgelt unterhalb der bundeseinheitlichen <br> Schwelle des unteren Entgeltbereichs (2.284 Euro), in Prozent</div>", size=15) %>%
  leaflet_legend(colors = colfunc(levels(geo$gruppe)),
                 lvl = levels(geo$gruppe),
                 group="2020") %>%
  leaflet_logo() %>%
  leaflet_quelle(Text="Quelle: <br> Bundesagentur (2021)") %>%
...
saveWidget(
  frameableWidget(fig), 
  file = paste0(getwd(), "/Output/Karten/", name_i2, ".html"),
  selfcontained=F
)
