...
fig <- leaflet_basis(geo) %>% 
  leaflet_text("Bruttolöhne, Dezember 2019", size=20, group="2019") %>%
  leaflet_text(lat=55.25, 
               "<div style='padding-top: 2%; line-height:90%'>Mittlere monatliche Bruttoarbeitsentgelte der 
  sozialversicherungspflichtig Vollzeitbeschäf- <br>tigten der Kerngruppe, in Euro</div>", size=15) %>%
  ...
  leaflet_polygons(
    ...
    k=2, # Nachkommastellen
    group="2019", suffix="Euro"
  ) 
...
fig <- fig %>%
  leaflet_text("Bruttolöhne, Dezember 2018", size=20, group="2018") %>%
  leaflet_polygons(
    labvar = 1, 
    colors=colfunc(levels$geo_gruppe),
    bg = colfunc(levels(geo$gruppe)),
...
fig <- fig %>%
  leaflet_text("Bruttolöhne, Dezember 2017", size=20, group="2017") %>%
  leaflet_polygons(
    labvar = 1, 
    colors=colfunc(levels$geo_gruppe),
    bg = colfunc(levels(geo$gruppe)),
...
fig <- fig %>%
  leaflet_text("Bruttolöhne, Dezember 2016", size=20, group="2016") %>%
  leaflet_polygons(
    labvar = 1, 
    colors=colfunc(levels$geo_gruppe),
    bg = colfunc(levels(geo$gruppe)),
...
if(ebene %in% c("KRS", "AMR")) {
  for(b in as.character(2019:2016)) {
    fig <- fig %>%
      addPolygons(
        data = geo_land,
...
fig <- fig %>%
  addLayersControl(
    baseGroups=c("2019", "2018", "2017", "2016"),
